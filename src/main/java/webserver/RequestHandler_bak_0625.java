...
public class RequestHandler_bak_0625 extends Thread {
	private static final Logger log = LoggerFactory.getLogger(RequestHandler_bak_0625.class);

	private Socket connection;

	public RequestHandler_bak_0625(Socket connectionSocket) {
	...
	public void run() {
		...
		try (InputStream in = connection.getInputStream(); OutputStream out = connection.getOutputStream()) {
			...
			/*
			 * 19.06.22 5.3) HttpRequest 사용으로 필요 상실 
			...
			if (("/user/create").equals(path)) {
				...
				*/
				...
				// if(!Boolean.parseBoolean(headerMap.get("logined"))) { // refactoring으로 삭제
				if (!isLogin(request.getHeaders("Cookie"))) {
					// responseResource(dos, "/user/login.html"); // 06.25 5.2.2) HttpResponse 사용으로 리팩토링
			...
			*/
			 ...
			 * 
			 * // 다른 응답 
			 * if (!"".equals(url)) { body = Files.readAllBytes(new File("./webapp"+ url).toPath()); }
			 * response200Header(dos, body.length); 
			 * responseBody(dos, body);
			 */
		} catch (IOException e) {
	...
	*/
	 ...
	 * 최종수정일 : 19.06.25 
	 * 
	 * 수정   이력 : 19.06.19) 요구사항 1.5 - 로그인하기
	 * 			19.06.22) HttpRequest 구현 및 사용
	 * 			19.06.25) HttpResponse 구현 및 사용
	 * 			19.06.25) Controller로 이전
	 ...
	 * 용	도 : 회원가입 
	 * 최종수정일 : 19.06.25 
	 * 수정   이력 : 19.06.17) 요구사항 2.1 - GET 방식으로 회원가입하기 
	 * 			19.06.19) 요구사항 2.2 - POST 방식으로 회원가입하기 / 
	 * 			19.06.19) 요구사항 2.5 - 로그인 하기(데이터베이스에 회원 정보 등록) : private String signIn(String url, int contentLength) throws IOException {
	 * 			19.06.22) 5.2.1 - HttpRequest 리팩토링으로 인해 기능 변경 : private String signIn(BufferedReader br, int contentLength) throws IOException {
	 * 			19.06.25) 5.2.2 - HttpResponse 구현 및 사용
	 * 			19.06.25) 5.2.3 - Controller로 이전
	 */
